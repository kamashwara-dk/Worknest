...
// Inner editor component — receives initial values as props and manages its own state.
// Using a separate component with key={doc.id} ensures state resets cleanly when doc loads.
...
function DocumentEditorInner({
  ...
  return (
    <motion.div variants={fadeUp} initial="hidden" animate="visible" className="max-w-5xl mx-auto">
      {/* Header */}
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {isDirty && <span className="text-xs text-amber-400 hidden sm:block">Unsaved changes</span>}
          <button
            onClick={() => togglePublic.mutate({ id })}
            className="p-2 rounded-lg hover:bg-white/5 text-zinc-400 hover:text-white transition-colors"
            title={isPublic ? 'Make private' : 'Make public'}
          >
            {isPublic ? <Globe size={16} className="text-secondary" /> : <Lock size={16} />}
          </button>
          <button
            onClick={handleSave}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        {tags.map((tag) => (
          <span key={tag} className="flex items-center gap-1 text-xs px-2 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
            <Tag size={10} />
            {tag}
      ...
      <div className="glass-card rounded-xl overflow-hidden min-h-[600px] flex flex-col">
        <DocumentEditor content={content} onChange={handleContentChange} placeholder="Start writing your document..." />
      </div>
...
export default function DocumentPage({ params }: DocumentPageProps) {
  ...
  // key={doc.id} ensures DocumentEditorInner remounts (resetting state) if the doc changes
  return (
    <DocumentEditorInner
      key={doc.id}
      id={doc.id}
