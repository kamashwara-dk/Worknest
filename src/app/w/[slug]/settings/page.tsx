...
import {
  Hash, Link2, Copy, RefreshCw, Plus, Trash2, ToggleLeft, ToggleRight,
  Check, ExternalLink, Clock, Users, ChevronDown, ChevronUp,
  Loader2, LogOut, AlertTriangle, Shield,
} from 'lucide-react';
      ...
      {isOwner ? (
        <>
          ...
          <DeleteWorkspaceSection
            ...
            onDeleted={clearWorkspace}
          />
        </>
      ) : (
        /* Non-owners see a locked notice for invite tools */
        <OwnerOnlyNotice />
      )}
...
function OwnerOnlyNotice() {
  return (
    <div className="bg-[#0D1F35] border border-[#1E3A5F] rounded-xl p-8 text-center">
      ...
      <p className="text-[#E8F0F8] font-medium">Owner access required</p>
      <p className="text-[#7A9BBF] text-sm mt-1 max-w-xs mx-auto">
        Invite links and join codes are managed exclusively by the workspace owner.
      </p>
    </div>
  );
}
    ...
    <section className="bg-[#0D1F35] border border-[#1E3A5F] rounded-xl overflow-hidden">
        ...
        {data && (
          ...
          >
            {toggle.isPending ? <Loader2 className="w-4 h-4 animate-spin" />
              : data.joinCodeEnabled ? <ToggleRight className="w-5 h-5 text-[#178582]" />
      ...
      <div className="px-6 py-5">
        {isLoading ? (
          ...
          <>
            <div className={`flex items-center gap-3 bg-[#112540] border rounded-xl px-5 py-4 ${data.joinCodeEnabled ? 'border-[#1E3A5F]' : 'border-[#1E3A5F] opacity-50'}`}>
            ...
            <div className="flex items-center justify-between mt-3">
              <p className="text-xs text-[#7A9BBF]">
                {data.joinCodeEnabled ? 'Anyone with this code can join as a Member' : 'Join code is disabled'}
              </p>
              <button
            ...
            <div className="mt-4 bg-[#112540] rounded-lg px-4 py-3 text-xs text-[#7A9BBF] space-y-1">
              ...
              <p>1. Copy the code and send it to your team member</p>
...
// ─── Invite Links section (OWNER only) ───────────────────────────────────────
...
function InviteLinksSection() {
  ...
  const create = trpc.workspaces.inviteLinks.create.useMutation({
    onSuccess: () => {
      ...
      setShowCreate(false); setLabel(''); setMaxUses(''); setExpiresInDays('');
  ...
  function handleCopy(token: string, id: string) {
    copyToClipboard(`${APP_URL}/invite/${token}`, 'Invite link copied');
    setCopiedId(id);
  ...
  return (
    <section className="bg-[#0D1F35] border border-[#1E3A5F] rounded-xl overflow-hidden">
      ...
      {showCreate && (
        ...
        >
          <form onSubmit={(e) => { e.preventDefault(); create.mutate({ label: label.trim() || undefined, maxUses: maxUses ? parseInt(maxUses) : undefined, expiresInDays: expiresInDays ? parseInt(expiresInDays) : undefined }); }} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-[#7A9BBF] mb-1.5">Label <span className="opacity-60">(optional)</span></label>
              <input type="text" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Engineering team, Q3 hiring" maxLength={80}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-[#7A9BBF] mb-1.5 flex items-center gap-1"><Users className="w-3 h-3" /> Max uses</label>
                <input type="number" value={maxUses} onChange={(e) => setMaxUses(e.target.value)} placeholder="Unlimited" min={1}
                  className="w-full bg-[#0D1F35] border border-[#1E3A5F] rounded-lg px-3 py-2 text-sm text-[#E8F0F8] placeholder-[#7A9BBF]/60 focus:outline-none focus:border-[#178582] transition-colors" />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#7A9BBF] mb-1.5 flex items-center gap-1"><Clock className="w-3 h-3" /> Expires in (days)</label>
                <input type="number" value={expiresInDays} onChange={(e) => setExpiresInDays(e.target.value)} placeholder="Never" min={1} max={365}
                  className="w-full bg-[#0D1F35] border border-[#1E3A5F] rounded-lg px-3 py-2 text-sm text-[#E8F0F8] placeholder-[#7A9BBF]/60 focus:outline-none focus:border-[#178582] transition-colors" />
              </div>
            </div>
            <button type="submit" disabled={create.isPending}
              className="flex items-center gap-1.5 bg-[#178582] hover:bg-[#178582]/90 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
              {create.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} Create Link
            </button>
          </form>
        </motion.div>
      )}
      ...
      <div className="px-6 py-4 space-y-3">
        {isLoading ? (
          <div className="space-y-3">{[1,2].map((i) => <div key={i} className="h-16 bg-[#112540] rounded-lg animate-pulse" />)}</div>
          ...
          <>
            {activeLinks.map((link) => (
              <LinkRow key={link.id} link={link} copiedId={copiedId}
                onCopy={() => handleCopy(link.token, link.id)}
                onDeactivate={() => deactivate.mutate({ id: link.id })}
                onDelete={() => deleteMutation.mutate({ id: link.id })}
                isDeactivating={deactivate.isPending} isDeleting={deleteMutation.isPending} />
            ))}
            {inactiveLinks.length > 0 && (
              <InactiveAccordion links={inactiveLinks}
...
function Badge({ children, color }: { children: React.ReactNode; color: 'red' | 'amber' | 'gray' }) {
  const cls = { red: 'bg-red-500/10 text-red-400 border-red-500/20', amber: 'bg-amber-500/10 text-amber-400 border-amber-500/20', gray: 'bg-[#1E3A5F] text-[#7A9BBF] border-[#1E3A5F]' };
  return <span className={`text-xs px-2 py-0.5 rounded-full border ${cls[color]}`}>{children}</span>;
}

function LinkRow({ link, copiedId, onCopy, onDeactivate, onDelete, isDeactivating, isDeleting }: {
  link: LinkData; copiedId: string | null;
  onCopy: () => void; onDeactivate: () => void; onDelete: () => void;
  isDeactivating: boolean; isDeleting: boolean;
}) {
  const url = `${APP_URL}/invite/${link.token}`;
  const isCopied = copiedId === link.id;
  const isExpired = link.expiresAt ? new Date(link.expiresAt) < new Date() : false;
  const isMaxed = link.maxUses !== null && link.useCount >= link.maxUses;
  const isValid = link.isActive && !isExpired && !isMaxed;

  return (
    <div className={`border rounded-xl p-4 ${isValid ? 'border-[#1E3A5F] bg-[#112540]' : 'border-[#1E3A5F]/50 bg-[#112540]/50 opacity-60'}`}>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="text-sm font-medium text-[#E8F0F8] truncate">{link.label ?? 'Invite link'}</span>
          ...
          <div className="flex items-center gap-3 flex-wrap text-xs text-[#7A9BBF]">
            <span className="flex items-center gap-1"><Users className="w-3 h-3" />{link.maxUses ? `${link.useCount} / ${link.maxUses} uses` : `${link.useCount} uses`}</span>
            {link.expiresAt && (
              <span className="flex items-center gap-1"><Clock className="w-3 h-3" />
                {isExpired ? `Expired ${formatDistanceToNow(new Date(link.expiresAt), { addSuffix: true })}` : `Expires ${formatDistanceToNow(new Date(link.expiresAt), { addSuffix: true })}`}
              </span>
            )}
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={onCopy} disabled={!isValid} className="p-2 rounded-lg hover:bg-[#1E3A5F] text-[#7A9BBF] hover:text-[#E8F0F8] transition-colors disabled:opacity-40 disabled:cursor-not-allowed" title="Copy link">
            {isCopied ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
          </button>
          <a href={url} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-[#1E3A5F] text-[#7A9BBF] hover:text-[#E8F0F8] transition-colors" title="Open link">
            <ExternalLink className="w-4 h-4" />
          </a>
          {isValid && (
            <button onClick={onDeactivate} disabled={isDeactivating} className="p-2 rounded-lg hover:bg-[#1E3A5F] text-[#7A9BBF] hover:text-amber-400 transition-colors" title="Deactivate">
              {isDeactivating ? <Loader2 className="w-4 h-4 animate-spin" /> : <ToggleRight className="w-4 h-4" />}
            </button>
          )}
          <button onClick={onDelete} disabled={isDeleting} className="p-2 rounded-lg hover:bg-[#1E3A5F] text-[#7A9BBF] hover:text-red-400 transition-colors" title="Delete">
...
function InactiveAccordion({ links, onDelete, isDeleting }: { links: LinkData[]; onDelete: (id: string) => void; isDeleting: boolean }) {
  const [open, setOpen] = useState(false);
  return (
    <div>
      <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 text-xs text-[#7A9BBF] hover:text-[#E8F0F8] transition-colors py-1">
      ...
      {open && (
        <div className="mt-2 space-y-2">
          {links.map((link) => (
            <LinkRow key={link.id} link={link} copiedId={null}
              onCopy={() => {}} onDeactivate={() => {}} onDelete={() => onDelete(link.id)}
              isDeactivating={false} isDeleting={isDeleting} />
          ))}
        </div>
      )}
    </div>
...
function DeleteWorkspaceSection({ workspaceName, onDeleted }: { workspaceName: string; onDeleted: () => void }) {
  ...
  const canConfirm = confirmText === workspaceName;

  return (
    <section className="bg-[#0D1F35] border border-red-500/30 rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-red-500/20">
        <div className="w-9 h-9 rounded-lg bg-red-500/10 flex items-center justify-center shrink-0">
          <Trash2 className="w-4 h-4 text-red-400" />
        </div>
        <div className="flex-1">
          <p className="font-semibold text-[#E8F0F8]">Delete Workspace</p>
          <p className="text-xs text-[#7A9BBF]">Permanently delete this workspace and all its data</p>
        </div>
      </div>
        ...
        {!confirming ? (
          ...
          <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-[#E8F0F8] text-sm">This action is irreversible</p>
                <p className="text-xs text-[#7A9BBF] mt-1">
                  All workspace data will be permanently deleted. Type <span className="font-mono text-red-400 font-bold">{workspaceName}</span> to confirm.
                </p>
              </div>
            </div>
            <input
              ...
              className="w-full bg-[#0D1F35] border border-red-500/30 rounded-lg px-4 py-2.5 text-sm text-[#E8F0F8] placeholder-[#7A9BBF]/60 focus:outline-none focus:border-red-500/60 transition-colors"
              autoFocus
            />
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-red-500 hover:bg-red-600 disabled:opacity-40 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors"
              >
...
// ─── Leave Workspace section (non-owners) ─────────────────────────────────────

function LeaveWorkspaceSection({ workspaceName, onLeft }: { workspaceName: string; onLeft: () => void }) {
  const router = useRouter();
  const [confirming, setConfirming] = useState(false);
    ...
    <section className="bg-[#0D1F35] border border-red-500/20 rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-red-500/10">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-[#7A9BBF]">Remove yourself from <span className="text-[#E8F0F8]">{workspaceName}</span></p>
        </div>
      </div>
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-[#E8F0F8] text-sm">
                  Leave <span className="text-red-400">{workspaceName}</span>?
                </p>
                <p className="text-xs text-[#7A9BBF] mt-1">Your access will be removed immediately.</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button
              ...
              >
                {leaveMutation.isPending
                  ...
                  : <><LogOut className="w-4 h-4" /> Yes, leave</>}
              </button>
              <button onClick={() => setConfirming(false)} disabled={leaveMutation.isPending}
                className="px-4 py-2 rounded-lg text-sm text-[#7A9BBF] hover:text-[#E8F0F8] transition-colors">
                Cancel
              </button>
            </div>
